...
# Función para cargar datos FER2013
@st.cache_data
def load_fer2013_data():
    """Carga los datos de FER2013"""
    try:
        data_loader = EmotionDataLoader(data_path='data/fer2013.csv')
        data = data_loader.preprocess_data()

        return data_loader, data
    except Exception as e:
        st.error(f"Error al cargar los datos de FER2013: {e}")
        return None, None

# Función para mostrar contenido de FER2013
def show_emotion_content():
    """Muestra el contenido para el dataset FER2013"""
    st.markdown("## 😊 FER2013 - Emociones")
    st.markdown("Dataset con 35,837 imágenes de expresiones faciales clasificadas en 7 emociones.")

    # Sub-pestañas para FER2013
    tabs = st.tabs(["📊 Dataset", "🚀 Entrenamiento", "📊 Evaluación", "🔮 Predicciones"])

    # Estado de carga de datos
    data_key = "fer2013_data_loaded"
    if data_key not in st.session_state:
        st.session_state[data_key] = False
    ...
    if not st.session_state[data_key]:
        col1, col2 = st.columns([1, 3])
        with col1:
            if st.button("📥 Cargar Datos FER2013", type="primary", key="load_fer2013_btn"):
                with st.spinner("Cargando datos de FER2013..."):
                    try:
                        data_loader, data = load_fer2013_data()

                        if data_loader and data:
                            st.session_state["fer2013_data_loader"] = data_loader
                            st.session_state["fer2013_data"] = data
                            st.session_state[data_key] = True
                            st.success("✅ Datos de FER2013 cargados exitosamente!")
                            st.rerun()
                        else:
                            st.error("❌ Error al cargar los datos de FER2013")
                    except Exception as e:
                        st.error(f"❌ Error al cargar datos: {str(e)}")
            else:
                st.info("💡 Haz clic en 'Cargar Datos FER2013' para comenzar")
                return
    ...
    if st.session_state[data_key]:
        ...
        with tabs[0]:  # Dataset
            show_emotion_dataset_section(data_loader, data)

        with tabs[1]:  # Entrenamiento
            show_emotion_training_section()

        with tabs[2]:  # Evaluación
            eval_key = "fer2013_eval_active"
            if st.session_state.get(eval_key, False) or st.button("🔍 Ejecutar Evaluación FER2013", key="fer2013_eval_btn"):
                st.session_state[eval_key] = True
                show_emotion_evaluation_section(data_loader, data)
            else:
                st.info("Haz clic en 'Ejecutar Evaluación' para ver las métricas del modelo.")

        with tabs[3]:  # Predicciones
            show_emotion_predictions_section()
...
main_tabs = st.tabs(["🔢 MNIST - Dígitos", "😊 FER2013 - Emociones"])

with main_tabs[0]:  # MNIST
    show_tab_content("MNIST", MNISTCNN, (28, 28, 1))

with main_tabs[1]:  # FER2013
