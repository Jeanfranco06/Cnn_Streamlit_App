...
class EmotionDataLoader:
    ...
    def _preprocess_expw(self, test_size=0.2, validation_split=0.1, max_samples=None):
        ...
        if len(X) == 0:
            ...
            # Create a more realistic synthetic dataset for testing
            import numpy as np

            # Create more samples per emotion for better training
            emotions = list(self.emotions.keys())
            X = []
            labels = []

            # Generate more diverse samples
            samples_per_emotion = 50  # More samples for better learning

            for emotion_idx in emotions:
                for i in range(samples_per_emotion):
                    # Create a 48x48 grayscale image with more realistic features
                    img = np.random.randint(0, 50, (48, 48), dtype=np.uint8)  # Base noise

                    # Add emotion-specific features with variations
                    if emotion_idx == 0:  # angry - sharp angles, furrowed brows
                        # Diagonal lines with noise
                        for j in range(0, 48, 3):
                            if j < 48:
                                img[j, min(j + np.random.randint(-2, 3), 47)] = np.random.randint(150, 255)
                        # Add some horizontal stress lines
                        img[15:20, 10:38] = np.random.randint(100, 200, (5, 28))

                    elif emotion_idx == 1:  # disgust - wrinkled nose, narrowed eyes
                        # Create wrinkled pattern
                        for y in range(20, 35, 2):
                            for x in range(15, 33, 2):
                                if np.random.random() > 0.5:
                                    img[y + np.random.randint(-1, 2), x + np.random.randint(-1, 2)] = np.random.randint(120, 180)
                        # Add some shadow under eyes
                        img[10:15, 12:36] = np.random.randint(80, 130, (5, 24))

                    elif emotion_idx == 2:  # fear - wide eyes, raised eyebrows
                        # Wide eye areas (lighter)
                        img[12:18, 8:18] = np.random.randint(180, 220, (6, 10))  # Left eye area
                        img[12:18, 30:40] = np.random.randint(180, 220, (6, 10))  # Right eye area
                        # Raised eyebrows
                        for x in range(5, 43):
                            if np.random.random() > 0.7:
                                img[8 + np.random.randint(-1, 2), x] = np.random.randint(140, 190)

                    elif emotion_idx == 3:  # happy - upturned mouth, smiling eyes
                        # Smile curve
                        for x in range(15, 33):
                            y_offset = int(5 * np.sin((x - 15) * np.pi / 18))
                            img[30 + y_offset, x] = np.random.randint(160, 220)
                            # Add some thickness to smile
                            if np.random.random() > 0.5:
                                img[31 + y_offset, x] = np.random.randint(140, 200)
                        # Slight eye crinkle
                        img[14:16, 10:16] = np.random.randint(120, 160, (2, 6))
                        img[14:16, 32:38] = np.random.randint(120, 160, (2, 6))

                    elif emotion_idx == 4:  # sad - downturned mouth, drooping eyes
                        # Sad mouth curve
                        for x in range(15, 33):
                            y_offset = int(-3 * np.sin((x - 15) * np.pi / 18))
                            img[32 + y_offset, x] = np.random.randint(140, 200)
                        # Drooping eyes
                        img[16:20, 10:16] = np.random.randint(100, 140, (4, 6))
                        img[16:20, 32:38] = np.random.randint(100, 140, (4, 6))
                        # Slight frown lines
                        img[25:27, 18:30] = np.random.randint(110, 150, (2, 12))

                    elif emotion_idx == 5:  # surprise - wide open eyes and mouth
                        # Very wide eyes
                        img[10:20, 8:18] = np.random.randint(200, 240, (10, 10))  # Left eye
                        img[10:20, 30:40] = np.random.randint(200, 240, (10, 10))  # Right eye
                        # Open mouth
                        img[28:38, 18:30] = np.random.randint(50, 80, (10, 12))
                        # Raised eyebrows
                        for x in range(5, 43, 2):
                            img[6 + np.random.randint(0, 3), x] = np.random.randint(130, 170)

                    elif emotion_idx == 6:  # neutral - balanced features
                        # Balanced eyes
                        img[12:18, 10:16] = np.random.randint(160, 190, (6, 6))  # Left eye
                        img[12:18, 32:38] = np.random.randint(160, 190, (6, 6))  # Right eye
                        # Neutral mouth
                        img[30:32, 20:28] = np.random.randint(140, 170, (2, 8))
                        # Even eyebrows
                        img[8:10, 8:40] = np.random.randint(120, 150, (2, 32))

                    # Add some random facial texture variation
                    for _ in range(np.random.randint(20, 50)):
                        y, x = np.random.randint(0, 48, 2)
                        img[y, x] = np.random.randint(100, 200)
