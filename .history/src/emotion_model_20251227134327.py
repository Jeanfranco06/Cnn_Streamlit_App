...
class EmotionClassifier:
    ...
    def preprocess_image(self, image):
        """Preprocesar imagen para predicción con manejo dual: sintético y real"""
        try:
            ...
            # Detectar si es imagen real o sintética
            is_real_image = self._is_real_face_image(image)
            ...
            if is_real_image:
                # Procesamiento ligero para imágenes reales
                # CLAHE suave para mejorar contraste sin distorsionar
                clahe = cv2.createCLAHE(clipLimit=1.5, tileGridSize=(8,8))
                processed = clahe.apply(gray.astype(np.uint8))

                # Suavizado muy ligero
                processed = cv2.GaussianBlur(processed, (3, 3), 0.5)

                # Redimensionar
                processed = cv2.resize(processed, (48, 48), interpolation=cv2.INTER_LINEAR)

                # Normalización simple para imágenes reales
                processed = processed.astype('float32') / 255.0

            else:
                # Para imágenes sintéticas o cuando no se detecta cara real
                # Usar procesamiento más agresivo que coincida con datos de entrenamiento
                ...
                # Normalización agresiva como en datos sintéticos
                processed = processed.astype('float32') / 255.0
                processed = (processed - processed.mean()) / (processed.std() + 1e-7)
                processed = np.clip(processed, -2, 2)
                processed = (processed + 2) / 4
            ...
            # Crear imagen dummy con patrón neutro
            dummy_image = np.full((1, 48, 48, 1), 0.5, dtype='float32')
            return dummy_image

    def _is_real_face_image(self, image):
        """Detectar si la imagen parece una cara real o es sintética"""
        try:
            ...
            # Criterios para detectar imagen real:
            # 1. Media no extrema (no todo negro/blanco como patrones sintéticos)
            # 2. Desviación estándar moderada
            # 3. Histograma con variación (no uniforme como formas geométricas)

            is_mean_realistic = 30 < mean_val < 225
            is_std_realistic = std_val > 20  # Más variación que formas geométricas simples
            is_hist_varied = hist_std > 50  # Histograma no uniforme

            # Si cumple la mayoría de criterios, es imagen real
            real_criteria = sum([is_mean_realistic, is_std_realistic, is_hist_varied])
            return real_criteria >= 2

        except:
            return False  # En caso de error, asumir sintética
