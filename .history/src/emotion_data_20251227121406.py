...
class EmotionDataLoader:
    ...
    def _preprocess_expw(self, test_size=0.2, validation_split=0.1, max_samples=None):
        ...
        if len(X) == 0:
            ...
            # Create 70 sample images (10 per emotion)
            emotions = list(self.emotions.keys())
            X = []
            labels = []

            for emotion_idx in emotions:
                for i in range(10):  # 10 samples per emotion
                    # Create a synthetic 48x48 grayscale image
                    ...
                    if emotion_idx == 0:  # angry - diagonal lines
                        ...
                        img[::4, ::4] = 255
                    elif emotion_idx == 2:  # fear - horizontal stripes
                        img[::8, :] = 255
                    elif emotion_idx == 3:  # happy - smile pattern
                        img[20:28, 10:38] = 255
                    elif emotion_idx == 4:  # sad - vertical lines
                        img[:, ::8] = 255
                    elif emotion_idx == 5:  # surprise - circles
                        center = (24, 24)
                        y, x = np.ogrid[:48, :48]
                        mask = (x - center[1])**2 + (y - center[0])**2 <= 10**2
                        img[mask] = 255
                    elif emotion_idx == 6:  # neutral - cross
                        img[24, :] = 255
                        img[:, 24] = 255
